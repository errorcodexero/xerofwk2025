...
public class SwerveRotateToAngle extends Command {
    ...
    public void execute() {
        ...
        if (!is_finished_) {
            db_.setControl(new ApplyRobotSpeeds().withSpeeds(new ChassisSpeeds(0, 0, Units.degreesToRadians(rotvel)))) ;
        } else {
            db_.setControl(new ApplyRobotSpeeds().withSpeeds(new ChassisSpeeds(0, 0, 0))) ;
        }
